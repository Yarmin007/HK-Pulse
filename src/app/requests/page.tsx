...
const MINIBAR_CATEGORIES = ['All', 'Bites', 'Sweets', 'Soft Drinks', 'Juices', 'Water', 'Beer', 'Spirits', 'Wines', 'Retail'];
...
export default function CoordinatorLog() {
  ...
  const [minibarCatalog, setMinibarCatalog] = useState<MinibarItem[]>([]);
  const [otherCategories, setOtherCategories] = useState<string[]>(["Maintenance", "Housekeeping"]); // Default fallback
  ...
  const fetchCatalog = async () => {
    // Fetch active items
    const { data } = await supabase.from('hsk_minibar_catalog').select('*').eq('active', true).order('micros_name');
    if (data) setMinibarCatalog(data);
  };

  const fetchSettings = async () => {
    // Fetch Categories from System Constants
    ...
    if (data && data.length > 0) {
      setOtherCategories(data.map(c => c.label));
    }
  };
  ...
  const visibleRecords = records.filter(r => {
     ...
     const matchesType = filterType === 'All' || (filterType === 'Minibar' && r.request_type === 'Minibar') || (filterType === 'Other' && r.request_type !== 'Minibar');
     const matchesStatus = filterStatus === 'All' || (filterStatus === 'Pending' && (!r.is_posted && !r.is_done)) || (filterStatus === 'Done' && (r.is_posted || r.is_done));
  ...
  const filteredCatalog = activeCategory === 'All' ? minibarCatalog : minibarCatalog.filter(i => i.category === activeCategory);
  ...
  return (
    <div className="min-h-screen bg-[#FDFBFD] font-antiqua text-[#6D2158] pb-32">
      ...
      <div className="sticky top-0 z-30 bg-white shadow-sm border-b border-slate-200">
         <div className="p-3 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 bg-slate-100 rounded-lg px-3 py-2 cursor-pointer">
               ...
               <input type="date" value={selectedDate} onChange={e => setSelectedDate(e.target.value)} className="bg-transparent text-sm font-bold text-slate-700 outline-none w-32"/>
            </div>
            <div className="flex-1 relative">
               <Search className="absolute left-3 top-2.5 text-slate-400" size={16}/>
               <input type="text" placeholder="Search Villa..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-bold outline-none focus:border-[#6D2158]"/>
            </div>
            ...
            <div key={record.id} className={`bg-white rounded-lg shadow-sm border-l-4 p-3 flex gap-3 animate-in slide-in-from-bottom-2 ${getStatusColor(record)}`}>
               ...
               <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-col gap-0.5">
                     {record.item_details.split(', ').map((item, i) => (
                        <span key={i} className="text-sm font-bold text-slate-700 block truncate leading-tight">{item.trim()}</span>
                     ))}
                  </div>
               </div>
               <div className="flex flex-col items-end justify-between gap-2">
                  ...
                  <div className="flex gap-1">
                     <button onClick={() => toggleStatus(record.id, 'is_sent')} className={`w-8 h-8 rounded-md flex items-center justify-center transition-all ${record.is_sent ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-400'}`}><Send size={14}/></button>
                     {record.request_type === 'Minibar' ? (
                        <button onClick={() => toggleStatus(record.id, 'is_posted')} className={`w-8 h-8 rounded-md flex items-center justify-center transition-all ${record.is_posted ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-400'}`}><CheckCircle2 size={14}/></button>
                     ) : (
                        <button onClick={() => toggleStatus(record.id, 'is_done')} className={`w-8 h-8 rounded-md flex items-center justify-center transition-all ${record.is_done ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-400'}`}><CheckCircle2 size={14}/></button>
                     )}
                  </div>
               </div>
            </div>
      ...
      {/* FAB */}
      <button onClick={openModal} className="fixed bottom-6 right-6 w-14 h-14 bg-[#6D2158] text-white rounded-full shadow-xl shadow-[#6D2158]/30 flex items-center justify-center hover:scale-110 active:scale-95 transition-all z-40"><Plus size={28}/></button>

      {/* MODAL */}
      {isModalOpen && (
         <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-end sm:items-center justify-center">
            <div className="bg-white w-full sm:w-[400px] h-[85vh] sm:h-auto sm:rounded-3xl rounded-t-3xl shadow-2xl flex flex-col animate-in slide-in-from-bottom-10">
               ...
               <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  ...
                  <div className="flex gap-2">
                     <input type="number" placeholder="Villa #" autoFocus className="w-20 bg-slate-50 border border-slate-200 rounded-xl text-center font-bold text-xl outline-none focus:border-[#6D2158] h-12" value={villaNumber} onChange={e => setVillaNumber(e.target.value)}/>
                     <input type="time" className="flex-1 bg-slate-50 border border-slate-200 rounded-xl text-center font-bold text-sm outline-none h-12" value={manualTime} onChange={e => setManualTime(e.target.value)}/>
                  </div>
                     ...
                     <>
                        {cart.length > 0 && (
                           <div className="flex flex-wrap gap-2">
                              {cart.map(i => (
                                 <button key={i.name} onClick={() => setCart(cart.filter(c => c.name !== i.name))} className="bg-[#6D2158] text-white px-3 py-1 rounded-lg text-xs font-bold flex items-center gap-1">
                        ...
                        <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                           {MINIBAR_CATEGORIES.map(c => (
                              <button key={c} onClick={() => setActiveCategory(c)} className={`px-4 py-2 rounded-lg text-xs font-bold uppercase whitespace-nowrap border ${activeCategory === c ? 'bg-slate-800 text-white border-slate-800' : 'bg-white border-slate-200 text-slate-400'}`}>{c}</button>
                           ))}
                        </div>
                        <div className="grid grid-cols-4 gap-2">
                           {filteredCatalog.map(item => {
                              ...
                              return (
                                 <button key={item.id} onClick={() => addToCart(item.micros_name)} className="aspect-square bg-slate-50 rounded-xl flex flex-col items-center justify-center gap-1 active:bg-slate-200">
                                    <IconComp size={18} className={item.color || 'text-slate-500'}/>
                                    <span className="text-[8px] font-bold text-slate-600 leading-none text-center px-1 line-clamp-2">{item.micros_name}</span>
                                 </button>
                              );
                           })}
                        </div>
                     ...
                     <>
                        <div className="grid grid-cols-2 gap-2">
                           {otherCategories.map(cat => (
                              <button key={cat} onClick={() => setOtherCategory(cat)} className={`py-3 rounded-xl border text-xs font-bold ${otherCategory === cat ? 'bg-[#6D2158] text-white border-[#6D2158]' : 'bg-white border-slate-200 text-slate-500'}`}>{cat}</button>
                           ))}
                        </div>
                        <textarea placeholder="Type details..." className="w-full h-24 p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold outline-none" value={customNote} onChange={e => setCustomNote(e.target.value)}/>
                     </>
                  )}
               </div>
