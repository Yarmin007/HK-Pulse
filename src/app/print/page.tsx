...
import { 
  ...
  ArrowRight, Search, Type, Trash2, FileText, Plane, History as HistoryIcon, Users, UserPlus, CalendarDays, Scissors, EyeOff
} from 'lucide-react';
import * as XLSX from 'xlsx';
import { PDFDocument, StandardFonts, PDFTextField, PDFCheckBox, PDFFont, rgb } from 'pdf-lib';
...
export default function PrintHubPage() {
  ...
  const fetchLeaveDetectionData = async () => {
      ...
      try {
          ...
          setDetectedLeaves(detected.filter(d => !ignored.includes(d.hash)));
  ...
  const ignoreLeaveBlock = (hash: string) => {
     ...
     showToast('success', "Leave alert ignored.");
  };
  ...
  const selectDetectedLeave = (leave: DetectedLeave) => {
      ...
      showToast('success', 'Form pre-filled from roster data!');
  };
      ...
      try {
          ...
          if (allLeavesArray.length > 0) {
              allLeavesArray.forEach(([code, count]) => {
                  ...
                  page.drawText(`${code}:`, { x: boxX + 15, y: boxY + 1, size: 9, font: boldFont });
          ...
          const breakdownStr = allLeavesArray.map(([k,v]) => `${v} ${k}`).join(', ');
